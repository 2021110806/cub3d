...
void	door_open(t_data *data)
{
	...
	if (check_user_direction(data, &check_door_location_x, \
	&check_door_location_y) == -1)
		return ;
	if (data->args.map.map[check_door_location_y][check_door_location_x] \
	== MINIMAP_CLOSE_DOOR)
	{
		data->args.map.map[check_door_location_y][check_door_location_x] \
		= MINIMAP_OPEN_DOOR;
	}
	else if (data->args.map.map[check_door_location_y][check_door_location_x] \
	== MINIMAP_OPEN_DOOR)
	{
		data->args.map.map[check_door_location_y][check_door_location_x] \
		= MINIMAP_CLOSE_DOOR;
